...
#Gernarate pac file including rules of local names(and ip names), proxyee-down, librabry names, Jpan names and china names for commo proxy usage.
...
var proxy_jp = "SOCKS5 127.0.0.1:1081; SOCKS 127.0.0.1:1080; DIRECT;";
var proxy_hfut = "SOCKS5 127.0.0.1:1082; DIRECT;";
...
var InetRangeList = [
    ...
    2130706432, 2139095040, //127.0.0.0/9
    2886729728, 2887778304, //169.254.0.0/16
    2886729728, 2887778304, //172.16.0.0/12
    3232235520, 3232301056, //192.168.0.0/16
    3758096384, 4026531840, //224.0.0.0/4
    4026531840, 4294967296  //240.0.0.0/4
];
...
function isInInetRangeList(ipRange, intIp) {
...
function isInetIP(strIp) {
    var intIp = convertAddress(strIp);
    if ( isInInetRangeList(InetRangeList, intIp) ) {
        return true;
    }
    return false;
}

var Local_Names_Matcher = new CombinedMatcher();
for (var i = 0; i < Local_Names.length; i++) {
    Local_Names_Matcher.add(Filter.fromText(Local_Names[i]));
}
...
var Lib_Names_Matcher = new CombinedMatcher();
for (var i = 0; i < Lib_Names.length; i++) {
    Lib_Names_Matcher.add(Filter.fromText(Lib_Names[i]));
}
...
var ChnList_Matcher = new CombinedMatcher();
for (var i = 0; i < ChnList.length; i++) {
    ChnList_Matcher.add(Filter.fromText(ChnList[i]));
}
...
function FindProxyForURL(url, host) {
    ...
    if (Local_Names_Matcher.matchesAny(url, host) instanceof BlockingFilter) {
        return direct;
    }
    if (Pd_Names_Matcher.matchesAny(url, host) instanceof BlockingFilter) {
        return proxy_pd;
    }
    if (Lib_Names_Matcher.matchesAny(url, host) instanceof BlockingFilter) {
        return proxy_hfut;
    }
    if (Jp_Names_Matcher.matchesAny(url, host) instanceof BlockingFilter) {
        return proxy_jp;
    }
    if (ChnList_Matcher.matchesAny(url, host) instanceof BlockingFilter) {
        return direct;
    }
    return proxy_us;
}
